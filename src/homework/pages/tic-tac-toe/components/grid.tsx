...
import { INITIAL_CELLS, PLAYER, getWinner, type Cells } from '../constants';
  ...
  return (
    <div className={tm('grid grid-rows-3 grid-cols-3 gap-1')}>
      {cells.map((cell, index) => {
        ...
        // 승리자가 있다면?
        if (winner) {
          // 승리 패턴(공식)에 해달하는 말판에 색칠 공부하기!!
          const [x, y, z] = winner.condition;
          if (index === x || index === y || index === z) {
            winnerClasses = 'outline-4 outline-amber-500 bg-amber-500/30';
          }
        }
        return (
          <Cell
